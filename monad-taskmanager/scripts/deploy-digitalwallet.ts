...
async function main() {
  ...
  const DigitalWalletKampusFactory = await ethers.getContractFactory("DigitalWalletKampus");
  ...
  const digitalWalletKampus: DigitalWalletKampus = await DigitalWalletKampusFactory.deploy({
    gasLimit: gasLimit
  });

  console.log("├── Transaction hash:", digitalWalletKampus.deploymentTransaction()?.hash);
  ...
  try {
    // const owner = await digitalWalletKampus.admin();
    // const taskCount = await digitalWalletKampus.taskCount();
    // const userTaskCount = await digitalWalletKampus.userTaskCount(deployer.address);
    ...
    // const fee = await digitalWalletKampus.calculateFee(100);
  ...
  // Get deployment cost
  const deploymentTx = digitalWalletKampus.deploymentTransaction();
  if (deploymentTx) {
  ...
  fs.writeFileSync(
    path.join(deploymentsDir, 'digitalWalletKampus-monad-testnet.json'),
    JSON.stringify(deploymentInfo, null, 2)
  );

  console.log("\n💾 Deployment info saved to: deployments/digitalWalletKampus-monad-testnet.json");
